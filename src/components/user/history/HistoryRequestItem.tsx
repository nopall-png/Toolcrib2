    ...
    <div className="bg-white border border-slate-200 rounded-2xl shadow-xs hover:shadow-md transition-all overflow-hidden">
      ...
      >
        <div className="flex items-center space-x-5 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-3">
              <span className="font-extrabold text-slate-900 text-xl">{request.requestNo}</span>
              {request.isNonStandard && (
                <span className="text-sm bg-indigo-100 text-indigo-700 font-bold px-2 py-0.5 rounded border border-indigo-200 uppercase tracking-wider hidden sm:inline-block">Non-Standard</span>
              )}
            </div>
        ...
        <div className="flex items-center justify-between sm:justify-end space-x-4">
          <span
            className={`inline-flex items-center space-x-2 text-sm font-bold px-4 py-2 rounded-full border ${statusColors[request.status] || 'bg-slate-100'}`}
          >
            {request.status === 'Pending' && <Clock className="w-5 h-5" />}
            {request.status === 'Approved' && <CheckCircle2 className="w-5 h-5" />}
            {request.status === 'Issued' && <PackageCheck className="w-5 h-5" />}
            {request.status === 'Returned' && <RotateCcw className="w-5 h-5" />}
            {request.status === 'Rejected' && <XCircle className="w-5 h-5" />}
            <span>{request.status}</span>
          </span>
          <div className="text-slate-400 hover:text-slate-600 bg-slate-100 p-2 rounded-lg transition-colors">
      ...
      {isExpanded && (
        <div className="px-6 pb-6 pt-4 border-t border-slate-100 bg-slate-50/50">
          ...
          <div className="space-y-4 mb-8">
            ...
            {request.isNonStandard ? (
              <div className="flex items-center space-x-4 bg-white border border-slate-200 p-4 rounded-xl shadow-xs">
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-slate-800 text-lg truncate">{request.nonStandardDetails?.toolName}</h4>
                  <p className="text-sm text-slate-500 truncate mt-1">Vendor: {request.nonStandardDetails?.vendorName} • Approver: {request.nonStandardDetails?.approver}</p>
                  <div className="mt-2">
                    <span className="text-sm font-bold text-slate-600 bg-slate-100 px-3 py-1 rounded">
                      1 Unit (Custom)
                    </span>
                  </div>
                </div>
              ...
              request.items.map((item, idx) => (
                <div key={idx} className="flex items-center space-x-4 bg-white border border-slate-200 p-4 rounded-xl shadow-xs">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-slate-500 font-mono mt-1">{item.toolCode}</p>
                  </div>
                  <div className="shrink-0">
                    <span className="text-base font-bold text-red-600 bg-red-50 border border-red-100 px-3 py-1.5 rounded-lg">
