  ...
  return (
    <div className={disabled ? 'opacity-50 pointer-events-none transition-opacity' : 'opacity-100 transition-opacity'}>
      <div className="flex items-center space-x-2 pb-3 mb-4 border-b border-slate-200 mt-6">
        ...
        <div>
          <h3 className="font-extrabold text-slate-900 text-base">Langkah 2: Pilih Approver <span className="text-red-500">*</span></h3>
          <p className="text-xs text-slate-500">Pilih atasan atau manager yang akan menyetujui pengajuan ini.</p>
        </div>
      </div>
        ...
        <table className="w-full text-left text-xs">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="p-3">Nama Approver</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {INITIAL_APPROVERS.map(approver => (
              ...
              >
                ...
                <td className="p-3 font-bold text-slate-900">{approver.name}</td>
                <td className="p-3 text-slate-600">{approver.role}</td>
                <td className="p-3 text-slate-600">{approver.department}</td>
              </tr>
            ))}
          </tbody>
        </table>
