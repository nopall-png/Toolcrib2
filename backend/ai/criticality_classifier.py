...
class CriticalityClassifier:
    def classify_critical_spares(self, df_sku: pd.DataFrame, df_trx: pd.DataFrame, df_machines: pd.DataFrame) -> pd.DataFrame:
        ...
        - Usage Score (35%): Seberapa sering barang dipakai
        - Lead Time Score (25%): Semakin lama lead time, semakin kritikal
        - Machine Score (40%): Sudah dihitung oleh data_provider berdasarkan impact_weight dari machine_tools
        """
        # 1. Usage Score (0-100)
        ...
        df_result = pd.merge(df_sku[['SKU_ID', 'Description', 'Unit_Price', 'Lead_Time_Days', 'Machine_Score']], usage_df, on='SKU_ID', how='left')
        df_result['Total_Usage'] = df_result['Total_Usage'].fillna(0)
        df_result['Usage_Score'] = df_result['Usage_Score'].fillna(0)
        df_result['Machine_Score'] = df_result['Machine_Score'].fillna(50) # Fallback if missing
